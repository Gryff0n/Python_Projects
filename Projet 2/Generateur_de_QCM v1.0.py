...
def menu_principal():
    try :
        ...
        print("veuillez créer les questions d'abord")
        creer_question()
        ...
        if not position > len(types_de_questions):
            ...
            if nb_question == 0:
                def answer_to_questions():
                    try:
                        answer = int(input("Veuillez entrer la bonne réponse :\n {number}  de {question} :\n 1 : {answer1}\n 2 : {answer2}\n 3 : {answer3}\n Votre réponse : ".format(number=nombre_a_convertir[position], question=questions_types[int(types_de_questions[position])], answer1=set_de_reponse[0], answer2=set_de_reponse[1], answer3=set_de_reponse[2])))
...
def répondre_QCM_Manuel():
    ...
    for i in range(len(L_question)):
        print('il reste ', len(L_question)-(i),' questions.')
        answer=0
        ...
        if answer==index_bonne_réponse:
            ...
            print(f'score : {score}/{len(L_question)}')
        else :
            print(f'mauvaise réponse ! la bonne réponse était {L_bonne_reponse[i]}.')
            print(f'score : {score}/{len(L_question)}')
    print(f"fin du QCM ! Votre score final est de {score}/{len(L_question)}")
    menu_principal()
